...
export class HermesPlanner extends BasePlanner {
  ...
  async plan(goal: string, context: Record<string, any> = {}): Promise<PlannerResult> {
    ...
    const prompt = {
      ...
      relevantSkills: skills.map(skill => ({ name: skill.name, description: skill.description, content: skill.content.slice(0, 8000) })),
      state: { currentGoal: this.state.currentGoal, recentActions: this.state.executionHistory.slice(-8), recentReflections: this.state.reflections.slice(-8), previousPlans: this.state.planningHistory.slice(-3) },
      context
    };
    ...
    try {
      const response = await client.chat([
        { role: 'system', content: [
          ...
          `Return at most ${this.options.maxSteps} steps. IDs must be unique and dependencies must reference earlier step IDs.`,
          'Prefer the smallest useful sequence; parallelizable independent steps may be marked independent.',
          ...
          'Return ONLY JSON matching: {"steps":[{"id":string,"description":string,"tool":string,"parameters":object,"dependsOn":string[],"estimatedDuration":number,"priority":number,"riskLevel":"low"|"medium"|"high","verificationRequired":boolean,"expectedOutcome":string}],"confidence":number,"reasoning":string,"alternatives":[]}'
        ].join(' ') },
      ...
      const normalized: PlannerResult = {
        steps: Array.isArray(parsed?.steps) ? parsed.steps.map((step: any, index: number) => this.normalizeStep(step, index)).filter(Boolean) as PlannedStep[] : [],
        confidence: Math.max(0, Math.min(1, Number(parsed?.confidence ?? 0.5))),
        reasoning: String(parsed?.reasoning || 'Model-generated plan'),
        alternatives: []
      };
  ...
  optimizePlan(plan: PlannerResult): PlannerResult {
    ...
    const depth = (id: string, seen = new Set<string>()): number => {
      ...
      seen.add(id);
      return 1 + Math.max(...step.dependsOn.map(dep => depth(dep, new Set(seen))));
    };
    const steps = [...plan.steps].sort((a, b) => depth(a.id) - depth(b.id) || b.priority - a.priority);
    const rank = new Map(steps.map((step, index) => [step.id, index]));
    for (const step of steps) step.dependsOn.sort((a, b) => (rank.get(a) ?? 0) - (rank.get(b) ?? 0));
    return { ...plan, steps };
  }
  ...
  private normalizeStep(step: any, index: number): PlannedStep | null {
    ...
    return {
      id: String(step.id || `plan_${Date.now()}_${index}`), description: String(step.description || `Execute ${step.tool}`), tool: step.tool,
      parameters: step.parameters && typeof step.parameters === 'object' ? step.parameters : {},
      dependsOn: Array.isArray(step.dependsOn) ? step.dependsOn.map(String) : [], estimatedDuration: Math.max(1, Number(step.estimatedDuration || 5)),
      priority: Math.max(0, Number(step.priority || 1)), riskLevel: risk, verificationRequired: step.verificationRequired !== false,
      expectedOutcome: String(step.expectedOutcome || 'Successful tool execution')
    };
  }
