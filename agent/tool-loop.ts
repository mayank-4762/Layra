...
export async function runToolLoop(initialMessages: ChatMessage[], registry: ToolRegistry, executor: ToolExecutor, options: ToolLoopOptions = {}): Promise<ToolLoopResult> {
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    let turn;
    try { turn = await client.chatWithTools(messages, { tools: registry.getAvailableTools(), toolChoice: 'auto', temperature: 0.2, maxTokens: 1600 }); }
    catch (error) { return { content: `Model error: ${error instanceof Error ? error.message : String(error)}`, rounds: round, toolCalls: totalCalls, stoppedReason: 'tool_error', messages }; }
