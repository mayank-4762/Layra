...
export class ProcessRegistry {
  ...
  start(command: string, cwd: string, env: NodeJS.ProcessEnv): ProcessSnapshot { assertSafeShellCommand(command); const shell = process.env.SHELL || (process.env.PREFIX ? path.join(process.env.PREFIX, 'bin', 'sh') : '/bin/sh'); const child = spawn(shell, ['-lc', command], { cwd, env, stdio: ['ignore', 'pipe', 'pipe'] }); const session: Session = { id: `proc_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`, command, cwd, child, status: 'running', output: '', truncated: false, startedAt: Date.now() }; child.stdout.setEncoding('utf8'); child.stderr.setEncoding('utf8'); const append = (chunk: string) => { session.output += chunk; if (session.output.length > this.maxOutputChars) { session.output = session.output.slice(-this.maxOutputChars); session.truncated = true; } }; child.stdout.on('data', append); child.stderr.on('data', append); child.once('error', () => this.finish(session, 'failed', null, null)); child.once('close', (code, signal) => this.finish(session, code === 0 ? 'completed' : 'failed', code, signal)); this.sessions.set(session.id, session); return this.snapshot(session); }
