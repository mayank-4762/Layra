...
export class ProcessRegistry {
  ...
  constructor() {
    this.maxOutputChars = Math.max(10_000, Math.min(2_000_000, Number(process.env.LAYRA_PROCESS_MAX_OUTPUT_CHARS || 200_000)));
    this.ttlMs = Math.max(60_000, Math.min(3_600_000, Number(process.env.LAYRA_PROCESS_TTL_MS || 1_800_000)));
  }
  ...
  start(command: string, cwd: string, env: NodeJS.ProcessEnv): ProcessSnapshot {
    ...
    const child = spawn(shell, ['-lc', command], { cwd, env, stdio: ['ignore', 'pipe', 'pipe'] });
    ...
    child.once('error', () => this.finish(session, 'failed', null, null));
    child.once('close', (code, signal) => this.finish(session, code === 0 ? 'completed' : 'failed', code, signal));
  ...
  async wait(id: string, timeoutMs = 30_000): Promise<ProcessSnapshot> { const session = this.require(id); if (session.status !== 'running') return this.snapshot(session); await new Promise<void>((resolve) => { const timer = setTimeout(resolve, timeoutMs); const done = () => { clearTimeout(timer); resolve(); }; session.child.once('close', done); }); return this.snapshot(session); }
