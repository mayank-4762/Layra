...
export class BrowserCdp {
  private readonly configuredWsUrl = process.env.LAYRA_CDP_WS_URL || '';
  private activeWsUrl = this.configuredWsUrl;
  private nextId = 1;

  isConfigured(): boolean { return Boolean(this.activeWsUrl); }

  async listTabs(): Promise<any[]> {
    const discovery = this.discoveryUrl();
    const response = await fetch(discovery, { headers: { 'User-Agent': 'Layra/4.0' } });
    ...
    return tabs.slice(0, Math.max(1, Math.min(100, Number(process.env.LAYRA_BROWSER_MAX_TABS || 25)))).map((tab: CdpTab) => ({ id: tab.id, type: tab.type, title: tab.title, url: tab.url, webSocketDebuggerUrl: tab.webSocketDebuggerUrl, active: tab.webSocketDebuggerUrl === this.activeWsUrl }));
  }

  async selectTab(idOrUrl: string): Promise<any> {
    const needle = String(idOrUrl || '').trim();
    if (!needle || needle.length > 1000) throw new Error('A tab id or URL is required');
    const tabs = await this.listTabs();
    const match = tabs.find(tab => tab.id === needle || tab.url === needle);
    if (!match?.webSocketDebuggerUrl) throw new Error(`CDP tab not found: ${needle}`);
    this.activeWsUrl = match.webSocketDebuggerUrl;
    return { id: match.id, title: match.title, url: match.url, active: true };
  }

  async navigate(url: string, signal?: AbortSignal): Promise<any> { validateHttpUrl(url); return this.command('Page.navigate', { url }, signal); }
  ...
  private discoveryUrl(): URL {
    const ws = this.configuredWsUrl || this.activeWsUrl;
    if (!ws) throw new Error('Browser CDP is not configured; set LAYRA_CDP_WS_URL');
    const parsed = new URL(ws.replace(/^ws:/, 'http:').replace(/^wss:/, 'https:'));
  ...
  private async command(method: string, params: Record<string, any>, signal?: AbortSignal): Promise<any> {
    if (!this.activeWsUrl) throw new Error('Browser CDP is not configured; set LAYRA_CDP_WS_URL');
    const wsUrl = this.activeWsUrl;
    ...
    return await new Promise((resolve, reject) => {
      ...
      socket.addEventListener('error', () => finish(new Error(`Unable to connect to browser CDP at ${wsUrl}`)));
    });
  }
