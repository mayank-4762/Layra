...
export class ToolExecutor {
  ...
  private async dispatch(toolName: string, parameters: Record<string, any>, startTime: number, signal?: AbortSignal): Promise<ToolExecutionResult> {
    switch (toolName) {
      ...
      case 'process.start': { if (process.env.LAYRA_ALLOW_SHELL !== 'true') return this.fail(toolName, 'Shell execution disabled; set LAYRA_ALLOW_SHELL=true to enable it', startTime); const command = String(parameters.command || '').trim(); if (!command) return this.fail(toolName, 'Command is required', startTime); const cwd = path.resolve(this.runtime.workspaceRoot, String(parameters.cwd || '.')); if (path.relative(this.runtime.workspaceRoot, cwd).startsWith('..')) return this.fail(toolName, 'Working directory escapes Layra workspace', startTime); const processResult = this.processRegistry.start(command, cwd, this.safeChildEnv()); return this.ok(processResult, startTime, { executor: 'native-process', background: true }); }
