...
export class ToolExecutor {
  ...
  private readonly browser: BrowserCdp;
  private readonly mcp: LayraMcpClient;
  private readonly scheduler: LayraScheduler;
  ...
  constructor(toolRegistry: ToolRegistry, state: AgentState) {
    ...
    this.browser = new BrowserCdp();
    this.mcp = new LayraMcpClient();
    this.scheduler = new LayraScheduler();
  }
  ...
  async execute(toolName: string, parameters: Record<string, any>, signal?: AbortSignal): Promise<ToolExecutionResult> {
    ...
    try { await this.toolRegistry.runAfterHooks(toolName, parameters, result, { signal, goal: this.state.currentGoal, sessionId: this.state.id }); }
    catch (error) { result = { ...result, success: false, error: `${result.error ? `${result.error}; ` : ''}after-hook failed: ${error instanceof Error ? error.message : String(error)}` }; }
  ...
  private async dispatch(toolName: string, parameters: Record<string, any>, startTime: number, signal?: AbortSignal): Promise<ToolExecutionResult> {
    switch (toolName) {
      ...
      case 'process.start': {
        if (process.env.LAYRA_ALLOW_SHELL !== 'true') return this.fail(toolName, 'Shell execution disabled; set LAYRA_ALLOW_SHELL=true to enable it', startTime);
        const command = String(parameters.command || '').trim(); assertSafeShellCommand(command);
        const cwd = path.resolve(this.runtime.workspaceRoot, String(parameters.cwd || '.'));
        const rel = path.relative(this.runtime.workspaceRoot, cwd);
        if (rel === '..' || rel.startsWith(`..${path.sep}`) || path.isAbsolute(rel)) return this.fail(toolName, 'Working directory escapes Layra workspace', startTime);
        return this.ok(this.processRegistry.start(command, cwd, this.safeChildEnv()), startTime, { executor: 'native-process', background: true });
      }
