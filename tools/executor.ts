...
import { registerPhase5MemoryTools } from './phase5-tool-registration';
...
export class ToolExecutor {
  ...
  constructor(toolRegistry: ToolRegistry, state: AgentState) { this.toolRegistry = toolRegistry; this.state = state; this.runtime = new NativeRuntime(); this.processRegistry = new ProcessRegistry(); this.memoryStore = new MemoryStore(); this.browser = new BrowserCdp(); this.mcp = new LayraMcpClient(); this.scheduler = new LayraScheduler(); this.delegator = new LayraDelegator(toolRegistry, this); this.android = new AndroidTermuxBridge(); this.androidControl = new AndroidControlBridge(); registerAndroidControlTools(this.toolRegistry, this.state); registerPhase5MemoryTools(this.toolRegistry, this.state); }
  ...
  private async dispatch(toolName: string, parameters: Record<string, any>, startTime: number, signal?: AbortSignal): Promise<ToolExecutionResult> {
    switch (toolName) {
      ...
      case 'memory.set': { const content = String(parameters.content || '').trim(); if (!content) return this.fail(toolName, 'Memory content is required', startTime); const kind = ['fact','lesson','preference','procedure','event'].includes(String(parameters.kind)) ? String(parameters.kind) as any : 'fact'; const record = await this.memoryStore.remember({ kind, content, tags: Array.isArray(parameters.tags) ? parameters.tags.map(String).slice(0, 20) : [], importance: Math.max(0, Math.min(10, Number(parameters.importance ?? 5))), source: parameters.source ? String(parameters.source) : 'Layra' }); this.state.longTermMemory.lastRecord = record; return this.ok(record, startTime, { executor: 'layra-memory', persistent: true, vaultPath: record.vaultPath }); }
      case 'memory.related': return this.ok(await this.memoryStore.related(String(parameters.id || ''), Math.max(1, Math.min(50, Number(parameters.limit || 8)))), startTime, { executor: 'layra-memory-graph', persistent: true });
      case 'memory.consolidate': await this.memoryStore.consolidate(); return this.ok({ records: await this.memoryStore.count(), vaultRoot: this.memoryStore.getVaultRoot() }, startTime, { executor: 'layra-memory-consolidation', persistent: true });
      case 'memory.rebuild_index': await this.memoryStore.rebuildVaultIndex(); return this.ok({ records: await this.memoryStore.count(), vaultRoot: this.memoryStore.getVaultRoot() }, startTime, { executor: 'layra-memory-index', persistent: true });
      case 'memory.vault_info': return this.ok({ enabled: process.env.LAYRA_OBSIDIAN_VAULT !== 'false', root: this.memoryStore.getVaultRoot(), format: 'obsidian-compatible-markdown' }, startTime, { executor: 'layra-memory-vault' });
