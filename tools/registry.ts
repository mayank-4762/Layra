...
/** Single capability/policy boundary inspired by Hermes tool validation and OpenClaw tool-surface assembly. */
export class ToolRegistry {
  ...
  grantBasicPermissions(grantedBy = 'system'): void { const safe = ['filesystem.read', 'filesystem.list', 'web.get', 'memory.get', 'memory.set', 'system.info', 'system.time']; for (const name of safe) this.grantPermission(name, grantedBy, 'Safe default capability'); if (process.env.LAYRA_ALLOW_LOCAL_WRITE === 'true') this.grantPermission('filesystem.write', grantedBy, 'Explicitly enabled by LAYRA_ALLOW_LOCAL_WRITE'); if (process.env.LAYRA_ALLOW_SHELL === 'true') { this.grantPermission('shell.execute', grantedBy, 'Explicitly enabled by LAYRA_ALLOW_SHELL'); for (const name of ['process.start','process.poll','process.list','process.kill','process.wait']) this.grantPermission(name, grantedBy, 'Explicitly enabled by LAYRA_ALLOW_SHELL'); } if (process.env.LAYRA_ALLOW_WEB_POST === 'true') this.grantPermission('web.post', grantedBy, 'Explicitly enabled by LAYRA_ALLOW_WEB_POST'); }
