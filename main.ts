...
function startScheduler(agent: HybridAgent): LayraScheduler {
...
async function runChat(): Promise<void> {
  ...
  const shutdown = () => { scheduler.stop(); readline.close(); agent.stop(); };
...
async function main(): Promise<void> {
  ...
  const shutdown = () => { scheduler.stop(); agent.stop(); console.log('Layra stopped.'); };
  process.once('SIGINT', shutdown); process.once('SIGTERM', shutdown);
