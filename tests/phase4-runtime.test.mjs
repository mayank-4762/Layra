...
test('Browser CDP selects a discovered tab and uses the selected websocket target', async () => {
  class FakeWebSocket {
    static sent = [];
    constructor(url) { this.url = url; this.listeners = {}; setTimeout(() => this.emit('open', {}), 0); }
    addEventListener(name, fn) { (this.listeners[name] ||= []).push(fn); }
    emit(name, value) { for (const fn of this.listeners[name] || []) fn(value); }
    send(payload) { FakeWebSocket.sent.push({ url: this.url, payload: JSON.parse(payload) }); const request = JSON.parse(payload); setTimeout(() => this.emit('message', { data: JSON.stringify({ id: request.id, result: { ok: true } }) }), 0); }
    close() {}
  }
