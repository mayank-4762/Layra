...
function clearLayraProviderEnv() { for (const key of ['LAYRA_MODEL_NAME','LAYRA_MODEL','LAYRA_MODEL_PROVIDER','LAYRA_MODEL_API_KEY_ENV','LAYRA_MODEL_API_ENDPOINT','LAYRA_MODEL_TEMPERATURE','LAYRA_MODEL_MAX_TOKENS','CUSTOM_MODEL_KEY']) delete process.env[key]; }
...
test('provider defaults are configurable and preserve a usable default', async () => {
  ...
  const config = getModelConfig(); assert.equal(config.provider, 'nvidia'); assert.equal(config.apiKeyEnv, 'NVIDIA_API_KEY'); assert.ok(config.apiEndpoint.startsWith('https://')); assert.ok(config.model.length > 0); assert.equal(createModelClient(), null);
});
